import React, { useState } from "react";
import Unity, { UnityContent } from "react-unity-webgl";
import "./unitygamesstyle.css";
import { Typography, Layout } from "antd";
import { useNavigate } from "react-router-dom";
import NavBar from "../../commons/NavBar";

const unityContent = new UnityContent(
  "build/FarmAnimals.json",
  "build/FarmAnimalsUnityLoader.js"
);

const { Title } = Typography;
const { Footer } = Layout;

export default function FarmAnimals() {
  const navigate = useNavigate();

  unityContent.on("GameOver", () => {
    setTimeout(function () {
      navigate("/");
    }, 7000);
  });

  return (
    <div className="game-container">
      <NavBar />
      <Title className="game-title"> ANIMALES PERDIDOS EN LA GRANJA </Title>
      <div className="game">
        <Unity unityContent={unityContent} />
      </div>
      <p className="game-instructions">
        AYUDA A MONSTY A ENCONTRAR LOS ANIMALES QUE SE SALIERON DE SUS CORRALES!{" "}
      </p>
      <p className="game-instructions">
        {" "}
        PARA MOVER A MONSTY USA LAS LETRAS W A S D, O LAS FLECHAS DE DIRECCION,
        PODES ACELERAR PRESIONANDO SHIFT Y SALTAR CON LA BARRA ESPACIADORA.
      </p>
      <Footer className="footer" style={{ backgroundColor: "#ffffff" }}>
        EduPlay GO, by hoolab® - 2024
      </Footer>
    </div>
  );
}
